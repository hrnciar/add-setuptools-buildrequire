...
# Patch loaned by Debian for fixing GLPK version check that doesn't account for major version.
Patch1: %{name}-fix-glpk.patch
...
%build
mkdir -p build
%cmake -Wno-dev -B build -DCMAKE_CXX_COMPILER_VERSION:STRING=$(gcc -dumpversion) \
...
%if 0%{?with_check}
make --jobs 4 OpenMS TOPP UTILS GUI -C build
%else
...
# Fix rpaths
chrpath -r %{_libdir}/OpenMS %{buildroot}%{_bindir}/*
