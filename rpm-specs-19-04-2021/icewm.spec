...
Recommends:     rxvt-unicode%{?_isa}

# Various additional useful tools
#   * Display resolution control
Suggests:       lxrandr%{?_isa}

#   * Launcher
Suggests:       nwg-launchers%{?_isa}

#   * Screenshot
Suggests:       gnome-screenshot%{?_isa}

Suggests:       network-manager-applet%{?_isa}
...
# Tweak default settings.  Requested by users which ask for simila AntiX look
# like UX.
sed -e 's!# TaskBarShowMailboxStatus=1 # 0/1!TaskBarShowMailboxStatus=0 # 0/1!' \
    -e 's!# TaskBarShowCPUStatus=1 # 0/1!TaskBarShowCPUStatus=0 # 0/1!' \
    -e 's!# TaskBarShowMEMStatus=1 # 0/1!TaskBarShowMEMStatus=0 # 0/1!' \
    -e 's!# TaskBarShowNetStatus=1 # 0/1!TaskBarShowNetStatus=0 # 0/1!' \
    %{buildroot}%{_datadir}/%{name}/preferences
