...
# Fix a case where we can fall off the end of a non-void function
Patch101:	value_via_Riemann_sum.patch
# Call the unlink system call instead of forking a shell
Patch102:	unlink.patch
# Rearrange #includes to avoid bad macro expansions with gcc 11
Patch103:	gcc11.patch
...
# build flags
sed -e 's|/lib/|/%{_lib}/|g' \
 -e "s|\(DYN_OPTION=shared\)|\1 -Wl,-soname=libLfunction.so.%{version} $RPM_LD_FLAGS -fopenmp|" \
...
pushd src
    ...
    %make_build all EXTRA="$RPM_OPT_FLAGS -fopenmp"
popd
