%global npm_name topojson-server
...
# License of topojson-server is ISC; others come from bundled dependencies. See
...
# Just as rollup did, we build a second bundle that exposes internals. (We
# could do this with rollup from the dev tarball in %%check, but unless we took
# great care, that would overwrite our main bundle, and we want to test with
# the same bundler we use in production.
mkdir -p build
esbuild --bundle --platform=node --sourcemap \
    --outfile=build/topojson-internals.js test/internals.js
...
%{_bindir}/geo2topo
%{_mandir}/man1/geo2topo.1*


%changelog
* Fri Feb 05 2021 Benjamin A. Beasley <code@musicinmybrain.net> - 3.0.1-2
- Add script to audit for dependencies with null licenses in %%check

* Sat Jan 23 2021 Benjamin A. Beasley <code@musicinmybrain.net> - 3.0.1-1
- Initial package
