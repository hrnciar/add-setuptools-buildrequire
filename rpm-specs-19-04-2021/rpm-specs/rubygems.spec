...
export RUBYOPT="-I$(ruby -e 'size = $LOAD_PATH.size; %w(rake minitest rdoc webrick).each {|r| require r}; puts $LOAD_PATH[...-size].join ?:')" \

# Also fake the SimpleCov for similar reasons as above.
...
GEM_COMMAND="skip test_realworld_{default_gem,upgraded_default_gem}" \
  rake TESTOPTS='- -v "-e /test_(manifest_is_up_to_date)/"'
