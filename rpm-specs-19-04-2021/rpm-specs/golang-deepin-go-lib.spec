...
%changelog
* Thu Mar 11 2021 Robin Lee <cheeselee@fedoraproject.org> - 5.6.0.9-1
- fix(proxy): 修复应用商店设置系统代理后，代理不生效的BUG。 (chenyunxiong)
- fix(appinfo): 调用 turbo invoker 时错误地修改了文件参数 (electricface)
- fix: deepin-turbo-invoker 进程退出后没有被回收 (electricface)
- perf(appinfo): 优化 startCommand 的内存占用 (electricface)
- fix(appinfo): 使用 turbo invoker 后应用代理失效 (electricface)
- fix(appinfo): 不正确使用 turbo-invoker (electricface)
- chore: debian 目录增加 api.json 文件 (wubowen)
- fix: fix Convey arg error (longqi)

* Tue Jan 26 2021 Fedora Release Engineering <releng@fedoraproject.org> - 5.6.0.2-2
