...
%global __provides_exclude_from ^%{_libdir}/gnome-books/
%global __requires_exclude ^(%%(find %{buildroot}%{_libdir}/gnome-books/ -name '*.so' | xargs -n1 basename | sort -u | paste -s -d '|' -))
...
Source0:        https://download.gnome.org/sources/gnome-books/40/gnome-books-%{version}.tar.xz
...
Requires:       libtracker-sparql%{?_isa}
...
- Update to 40.0
- Filter out provides for private libgd
- Add missing libtracker-sparql requires
