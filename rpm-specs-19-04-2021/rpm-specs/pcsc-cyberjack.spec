...
Source0:	https://support.reiner-sct.de/downloads/LINUX/V%{version_prefix}_%{version_suffix}/%{name}_%{version}.tar.gz
...
install -D -p -m 0644 %{SOURCE2} %{buildroot}/usr/lib/udev/rules.d/93-cyberjack.rules
sed -e 's/GROUP="pcscd"/GROUP="cyberjack"/' -i %{buildroot}/usr/lib/udev/rules.d/93-cyberjack.rules
touch -c -r %{SOURCE2} %{buildroot}/usr/lib/udev/rules.d/93-cyberjack.rules
