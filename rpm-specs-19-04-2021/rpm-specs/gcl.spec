...
# C library functions that succeed can still set errno to a nonzero value.
# The massert() macro asserts that errno is zero, which can be the wrong test.
# See https://bugzilla.redhat.com/show_bug.cgi?id=1916968
Patch513:       %{name}-2.6.12-massert.patch
...
Requires:       %{name} = %{version}-%{release}
Requires:       xemacs(bin) %{?_xemacs_version:>= %{_xemacs_version}}
