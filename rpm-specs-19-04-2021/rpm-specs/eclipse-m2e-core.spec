...
BuildRequires:  osgi(biz.aQute.bndlib)
...
# Relax dep on aqute
sed -i -e '/biz.aQute.bndlib/s/5.1.2/4.2.0/' org.eclipse.m2e.pde{,.ui}/META-INF/MANIFEST.MF

# Dep on jansi needs to be exact for compat packages
%pom_xpath_inject 'pom:dependency[pom:artifactId="jansi"]' '<version>1.18</version>' m2e-maven-runtime/org.eclipse.m2e.maven.runtime
...
sed -i -e '/executionEnvironment/d' pom.xml
...
for aid in org.eclipse.m2e.archetype.common \
  ...
  if [ "$aid" = "org.eclipse.m2e.maven.indexer" ] ; then
  xmvn -B -o install:install-file -DgroupId=org.eclipse.m2e -DartifactId=$aid -Dversion=1.17.1-SNAPSHOT -Dpackaging=jar \
    -Dmaven.repo.local=../${repo} -Dfile=../m2e-maven-runtime/$aid/target/$aid-1.17.1-SNAPSHOT.jar
  else
...
%mvn_build -j -f -- -Dmaven.repo.local=${repo} -DforceContextQualifier=$QUALIFIER
...
do
    %mvn_artifact -Dtype=eclipse-plugin m2e-maven-runtime/$mod/pom.xml m2e-maven-runtime/$mod/target/$mod-*-SNAPSHOT.jar
done
...
%changelog
* Fri Mar 12 2021 Mat Booth <mat.booth@redhat.com> - 1.17.2-1
- Update to latest upstream release

* Tue Jan 26 2021 Fedora Release Engineering <releng@fedoraproject.org> - 1.17.1-2
- Rebuilt for https://fedoraproject.org/wiki/Fedora_34_Mass_Rebuild

* Thu Jan 21 2021 Mat Booth <mat.booth@redhat.com> - 1.17.1-1
- Update to latest upstream release

* Mon Dec 14 2020 Jerry James <loganjerry@gmail.com> - 1.16.2-4
