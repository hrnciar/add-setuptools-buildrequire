...
%{__php} -d extension=modules/horde_lz4.so \
  %{_bindir}/phpunit \
%if "%{php_version}" > "8.0"
    --filter '^((?!(testCompressErrorConditionOne|testCompressErrorConditionTwo|testUncompressErrorConditionOne|testUncompressErrorConditionTwo)).)*$' \
%endif
    test
%endif
...
%{__ztsphp} -d extension=modules/horde_lz4.so \
  %{_bindir}/phpunit \
%if "%{php_version}" > "8.0"
    --filter '^((?!(testCompressErrorConditionOne|testCompressErrorConditionTwo|testUncompressErrorConditionOne|testUncompressErrorConditionTwo)).)*$' \
%endif
...
%changelog
* Fri Mar  5 2021 Remi Collet <remi@fedoraproject.org> - 1.0.10-18
- rebuild for https://fedoraproject.org/wiki/Changes/php80
- trivial fix for PHP 8

* Wed Jan 27 2021 Fedora Release Engineering <releng@fedoraproject.org> - 1.0.10-17
