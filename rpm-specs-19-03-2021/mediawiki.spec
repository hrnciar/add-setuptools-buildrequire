...
Source0: http://download.wikimedia.org/mediawiki/1.35/%{name}-%{version}.tar.gz
Source1: http://download.wikimedia.org/mediawiki/1.35/%{name}-%{version}.tar.gz.sig
...
BuildRequires: php-composer(zordius/lightncandy) >= 1.2.5
BuildRequires: php-pear(Mail) >= 1.4.1
BuildRequires: php-pear(Mail_Mime) >= 1.10.8
BuildRequires: php-pear(Mail_mimeDecode) >= 1.5.5
BuildRequires: php-pear(Net_SMTP) >= 1.9.1
...
Requires: php-composer(oojs/oojs-ui) >= 0.39.3
...
Requires: php-composer(zordius/lightncandy) >= 1.2.5
Requires: php-pear(Mail) >= 1.4.1
Requires: php-pear(Mail_Mime) >= 1.10.8
Requires: php-pear(Mail_mimeDecode) >= 1.5.5
Requires: php-pear(Net_SMTP) >= 1.9.1
...
Provides: bundled(php-composer-semver) = 1.7.2
Provides: bundled(php-guzzlehttp-guzzle) = 6.5.5
Provides: bundled(php-guzzlehttp-promises) = 1.4.0
Provides: bundled(php-guzzlehttp-psr7) = 1.7.0
Provides: bundled(php-james-heinrich-getid3) = 1.9.14
Provides: bundled(php-jakobo-hotp-php) = 1.0.1
Provides: bundled(php-pleonasm-bloom-filter) = 1.0.2
...
Provides: bundled(php-wikimedia-at-ease) = 2.0.0
Provides: bundled(php-wikimedia-common-passwords) = 0.2.0
...
Provides: bundled(php-wikimedia-ip-utils) = 1.0.0
Provides: bundled(php-wikimedia-less.php) = 3.0.0
Provides: bundled(php-wikimedia-object-factory) = 2.1.0
Provides: bundled(php-wikimedia-parsoid) = 0.12.1
...
Provides: bundled(php-wikimedia-remex-html) = 2.2.0
...
Provides: bundled(php-wikimedia-wrappedstring) = 3.2.0
Provides: bundled(php-wikimedia-xmp-reader) = 0.7.0
Provides: bundled(php-wikimedia-zest-css) = 1.1.3
...
%files
%doc FAQ HISTORY README.md README.RPM RELEASE-NOTES-1.35 UPGRADE CREDITS docs
%license COPYING
%{_datadir}/mediawiki
/var/www/wiki
