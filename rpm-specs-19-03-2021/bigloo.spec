...
Requires:       xemacs-filesystem >= %{?_xemacs_version}%{!?_xemacs_version:0}
...
# The 4.4a release omits a Java definition of $date-is-gmt, leading to build
# failure.  Patching date.scm leads to bigloo attempting to regenerate the C
# files, with bigloo, before the bigloo binary is built, leading to build
# failure.  Instead, we use sed to insert the missing definition, then
# carefully put the timestamp back the way we found it.  We don't want the C
# files to be regenerated, because this affects Java only.
sed -i.orig 's/method.*ISDST.*/&\n\t       (method static $date-is-gmt::bool (::date) "BGL_DATE_ISGMT")/' runtime/Llib/date.scm
touch -r runtime/Llib/date.scm.orig runtime/Llib/date.scm
rm runtime/Llib/date.scm.orig
...
export CFLAGS="%{optflags} -fwrapv -Wno-unused"
%if 0%{?__isa_bits} == 32
