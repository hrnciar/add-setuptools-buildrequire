...
%global test_artifacts_commit 77faecd45e85b14448667bc16af32ddfe6f8d42d
...
BuildRequires:  python3dist(attrs) >= 20.3
# Bundled version of cached-property is 1.5.2, but Fedora has an older version
BuildRequires:  python3dist(cached-property) >= 1.5.1
BuildRequires:  python3dist(cerberus) >= 1.3.2
BuildRequires:  python3dist(click) >= 7.1.2
BuildRequires:  python3dist(colorama) >= 0.4.4
BuildRequires:  python3dist(distlib) >= 0.3.1
BuildRequires:  python3dist(docopt) >= 0.6.2
BuildRequires:  python3dist(first) >= 2.0.2
BuildRequires:  python3dist(chardet) >= 3.0.4
BuildRequires:  python3dist(iso8601) >= 0.1.13
BuildRequires:  python3dist(jinja2) >= 2.11.2
BuildRequires:  python3dist(markupsafe) >= 1.1.1
BuildRequires:  python3dist(packaging) >= 20.4
BuildRequires:  python3dist(parse) >= 1.18
BuildRequires:  python3dist(pexpect) >= 4.8
BuildRequires:  python3dist(ptyprocess) >= 0.6
BuildRequires:  python3dist(pyparsing) >= 2.4.7
BuildRequires:  python3dist(python-dotenv) >= 0.15
BuildRequires:  python3dist(requests) >= 2.25
BuildRequires:  python3dist(semver) >= 2.13
BuildRequires:  python3dist(shellingham) >= 1.3.2
...
# Re-bundled because it is only used by pipenv
...
# TODO: Make this work
# ("passa" & "pep514tools" this has no known version and hence confuse pythonbundles.py)
