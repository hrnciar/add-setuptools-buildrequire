...
CC=%{mingw32_cc} LD=%{mingw32_target}-ld DLLWRAP=%{mingw32_target}-dllwrap _PYTHON_HOST_PLATFORM=mingw _PYTHON_SYSCONFIGDATA_NAME="_sysconfigdata__win32_" PYTHONHOME=%{mingw32_prefix} PYTHONPATH=\$PYTHONPATH:%{mingw32_py3_hostlibdir}:%{mingw32_py3_hostlibdir}/site-packages:\$pylibdynload:%{mingw32_py3_libdir}:%{mingw32_python3_sitearch} PYTHONPLATLIBDIR=lib /usr/bin/python3 "\$@"
EOF

cat > %{buildroot}%{_prefix}/%{mingw64_target}/bin/python3 <<EOF
#!/bin/sh
pylibdynload=\$(/usr/bin/python%{py_ver} -c 'import sysconfig; import os; print(os.path.join(sysconfig.get_path("platstdlib"), "lib-dynload"))')
CC=%{mingw64_cc} LD=%{mingw64_target}-ld DLLWRAP=%{mingw64_target}-dllwrap _PYTHON_HOST_PLATFORM=mingw _PYTHON_SYSCONFIGDATA_NAME="_sysconfigdata__win32_" PYTHONHOME=%{mingw64_prefix} PYTHONPATH=\$PYTHONPATH:%{mingw64_py3_hostlibdir}:%{mingw64_py3_hostlibdir}/site-packages:\$pylibdynload:%{mingw64_py3_libdir}:%{mingw64_python3_sitearch} PYTHONPLATLIBDIR=lib /usr/bin/python3 "\$@"
EOF
...
%%mingw32_py3_build() %%{expand:\\\\\\
  CFLAGS="${CFLAGS:-%%{mingw32_cflags}}" LDFLAGS="${LDFLAGS:-%%{mingw32_ldflags}}" \\\\\\
  %%{mingw32_python3} %%{py_setup} %%{?py_setup_args} build --executable="%%{mingw32_python3} %%{py3_shbang_opts}" -b build_mingw32 %%{?*}
}

%%mingw32_py3_install() %%{expand:\\\\\\
...
%%mingw64_py3_build() %%{expand:\\\\\\
  CFLAGS="${CFLAGS:-%%{mingw64_cflags}}" LDFLAGS="${LDFLAGS:-%%{mingw64_ldflags}}" \\\\\\
  %%{mingw64_python3} %%{py_setup} %%{?py_setup_args} build --executable="%%{mingw64_python3} %%{py3_shbang_opts}" -b build_mingw64 %%{?*}
}

%%mingw64_py3_install() %%{expand:\\\\\\
